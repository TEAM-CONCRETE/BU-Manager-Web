...
const IconPeople = ({ className }: { className?: string }) => (
  <svg
    ...
    className={className}
  >
    <g clipPath="url(#clip0_657_26367)">
      <path
...
export function WorkforceCard({ total, salaried, daily, attendanceToday }: WorkforceCardProps) {
  return (
    <div className="rounded-3xl border border-border bg-bg-surface p-6 dark:border-dark-border dark:bg-dark-bg-surface">
      ...
      <div className="mt-6 flex items-start gap-6">
        <div>
          <p className="text-sm text-text-subtle">총 근로자</p>
          <p className="text-4xl font-semibold text-text-strong dark:text-dark-text-strong">
            {total}
          </p>
        </div>
        <div className="flex-1 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <InfoBadge label="상용직" value={`${salaried}`} tone="solid" />
            <InfoBadge label="일용직" value={`${daily}`} />
          </div>
          <div className="rounded-2xl bg-green-50 px-4 py-3 text-sm text-green-800">
            금일 출근{" "}
            <span className="text-2xl font-semibold text-green-600">{attendanceToday}</span>
          </div>
        </div>
      </div>
    </div>
...
function InfoBadge({ label, value, tone }: { label: string; value: string; tone?: "solid" }) {
  const base =
    tone === "solid"
      ? "bg-brand-primary-soft/70 text-brand-primary-strong"
      : "bg-bg-subtle text-text-strong dark:bg-dark-bg-surface dark:text-dark-text-strong";
  return (
    <div className={`rounded-2xl px-4 py-3 text-center text-sm font-semibold ${base}`}>
      <p className="text-xs font-medium text-text-subtle dark:text-dark-text-base">{label}</p>
