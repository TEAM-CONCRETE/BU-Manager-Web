...
export default function CompanyDashboardPage() {
  const { data: siteData, isLoading: isSitesLoading, refetch: refetchSites } = useCompanySites();
  const sites = useMemo(() => siteData?.sites ?? [], [siteData]);
  ...
  useEffect(() => {
    ...
    const found = selectedSiteId && sites.some((site) => String(site.siteId) === selectedSiteId);
    if (!selectedSiteId || !found) {
      setSelectedSiteId(String(sites[0].siteId));
    }
  ...
  const siteMenuItems = useMemo<SidebarMenuItem[]>(() => {
    if (!sites.length) {
      return [
        {
          id: "empty",
          label: isSitesLoading ? "현장 목록을 불러오는 중..." : "등록된 현장이 없습니다",
          icon: buildingIcon,
          active: true,
        },
      ];
    ...
    const activeId = selectedSiteId ?? String(sites[0].siteId);
  ...
  }, [sites, selectedSiteId, isSitesLoading]);
  ...
  return (
    <SidebarLayout
      ...
      bottomAction={{
        label: "현장 추가",
        onClick: () => {
          // TODO: 현장 추가 플로우와 연결 후 성공 시 refetchSites 호출
          refetchSites();
        },
      }}
